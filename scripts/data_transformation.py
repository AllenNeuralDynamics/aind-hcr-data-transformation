...
def transform_pan_protein_data(
    ...
    for stack in stacks_to_process:
        ...
        output_path = Path(basic_job.job_settings.output_directory).joinpath(
            stack_name
        )
        ...
        for idx in range(C):
            ...
            if basic_job.job_settings.s3_location is not None:
                ...
                s3_channel_zgroup_file = f"{basic_job.job_settings.s3_location}/{stack_name}/.zgroup"
                print(
                ...
                ome_zarr_stack_path = output_path.joinpath(
                    ome_zarr_curr_stack_name
                )
                s3_stack_dir = (
                    f"{basic_job.job_settings.s3_location}/{stack_name}/"
                    f"{ome_zarr_curr_stack_name}"
                )
                print(f"Uploading {ome_zarr_stack_path} to {s3_stack_dir}")
...
if __name__ == "__main__":
    BASE_PATH = Path("/path/to/your/dataset")
    ...
    if input_source.exists():
        ...
        transform_pan_protein_data(
            ...
            num_partitions=1,
            s3_location="s3://some-bucket/test_data/protein_project",
        )
